...
import com.vast.verona.InternalException

sealed trait DataType {
  def name: String
  def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CValue
}

sealed abstract class NativeDataType(val name: String) extends DataType {
  override def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue
}
...
case class CustomType(className: String) extends DataType {
  ...
  def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CValue = CCustomValue(className, bs)
}

object DataType {

  def fromByteIterator(it: ByteIterator)(implicit byteOrder: ByteOrder): DataType = {
  ...
  private[this] def listType(valueType: NativeDataType): CollectionType = new CollectionType {
    ...
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CValue = CList(ParamCodecUtils.unpackListParam(bs).map(valueType.decode))
  }

  private[this] def setType(valueType: NativeDataType): CollectionType = new CollectionType {
    def name: String = s"SET[${valueType.name}]"

    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CValue = CSet(ParamCodecUtils.unpackListParam(bs).map(valueType.decode).toSet)
  }
...
object CustomType {
  def fromByteIterator(it: ByteIterator)(implicit byteOrder: ByteOrder): CustomType = CustomType(ParamCodecUtils.readString(it))
}
...
object NativeDataType {
  ...
  def fromId(id: Int): NativeDataType = {
    nativeTypeMap.get(id).getOrElse {
      throw new InternalException(s"Invalid type code $id returned from server - expected a native type ID.")
    }
  }
  ...
  object ASCIIType extends NativeDataType("ASCII") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CASCIIString(bs.decodeString("ASCII"))
  }

  object BigIntType extends NativeDataType("BIGINT") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CBigInt(bs.iterator.getLong)
  }

  object BlobType extends NativeDataType("BLOB") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CBlob(bs)
  }

  object BooleanType extends NativeDataType("BOOLEAN") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CBoolean(bs.head != 0)
  }

  object CounterType extends NativeDataType("COUNTER") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CCounter(bs.iterator.getLong)
  }

  object DecimalType extends NativeDataType("DECIMAL") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = {
      val it = bs.iterator
      val scale = it.getInt
  ...
  object DoubleType extends NativeDataType("DOUBLE") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CDouble(bs.iterator.getDouble)
  }

  object FloatType extends NativeDataType("FLOAT") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CFloat(bs.iterator.getFloat)
  }

  object IntType extends NativeDataType("INT") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CInt(bs.iterator.getInt)
  }

  object TimestampType extends NativeDataType("TIMESTAMP") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CTimestamp(new DateTime(bs.iterator.getLong))
  }
  ...
  object VarCharType extends NativeDataType("VARCHAR") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CVarChar(bs.utf8String)
  }

  object VarIntType extends NativeDataType("VARINT") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CVarInt(new JBigInteger(bs.toArray))
  }
  ...
  object InetAddressType extends NativeDataType("INET") {
    def decode(bs: ByteString)(implicit byteOrder: ByteOrder): CScalarValue = CInetAddress(InetAddress.getByAddress(bs.toArray))
  }
