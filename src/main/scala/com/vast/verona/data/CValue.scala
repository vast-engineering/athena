...
import com.vast.verona.client.pipeline.ByteStringUtils
  ...
  def toByteString(cv: CValue)(implicit byteOrder: ByteOrder): ByteString = {
    cv match {
      ...
      case CMap(values) =>
        ...
        val strings = new ArrayBuffer[ByteString](length)
        values.foreach {
          case (key, value) =>
      ...
      case CSet(values) =>
        val converted = values.map(toByteString(_))
        ParamCodecUtils.packParamByteStrings(converted, converted.size)
      case CCustomValue(_, _) => throw new InternalException("Custom values are not supported.")
    }
  }
...
object CBoolean {
  private[verona] val TRUE_BUFFER = ByteString(Array[Byte](1))
  private[verona] val FALSE_BUFFER = ByteString(Array[Byte](0))
}
